...
## 2026-02-28 - Event Delegation & Redundant DOM Queries
**Learning:** In a codebase using vanilla JavaScript, attaching event listeners in a `forEach` loop over a NodeList (e.g. `navLinks.forEach`) creates multiple anonymous functions and listener instances, which can increase memory usage and initialization time, especially as the number of elements grows. Furthermore, executing redundant `document.querySelectorAll` calls for elements already selected and stored in a variable wastes CPU cycles.
**Action:** Always use event delegation by attaching a single event listener to the parent container and checking `e.target.closest('selector')` to handle events for multiple child elements. Additionally, reuse existing NodeList variables instead of querying the DOM again for the same elements.
